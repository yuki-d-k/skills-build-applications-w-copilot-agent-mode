...
function Workouts() {
  ...
  useEffect(() => { const controller = new AbortController(); fetchEndpoint(getApiUrl('workouts'), controller.signal).then(setWorkouts).catch((cause) => { if (cause.name !== 'AbortError') setError(cause.message) }); return () => controller.abort() }, [])
