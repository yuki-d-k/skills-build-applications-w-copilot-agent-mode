...
function Teams() {
  ...
  useEffect(() => { const controller = new AbortController(); fetchCollection('teams', controller.signal).then(setTeams).catch((cause) => { if (cause.name !== 'AbortError') setError(cause.message) }); return () => controller.abort() }, [])
